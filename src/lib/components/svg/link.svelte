<script lang="ts">
	...
	$: {
		if (linkTextElement) {
			linkTextElement.style.transform = flipText === true ? 'rotate(180deg)' : '';
		}
		if (linkWeightElement) {
			linkWeightElement.style.transform = flipText === true ? 'rotate(180deg)' : '';
		}
	}
...
<div class="container" style="pointer-events: auto">
	...
	>
		...
		<line
			...
			x2={length - 2}
			y2={height / 2}
			stroke={linkText.includes('property')
				? '#a3a3a3'
				: linkText.includes('asset')
					? '#525252'
					: '#1e40af'}
