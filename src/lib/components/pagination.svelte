...
{#if state.hasMore || state.totalItems !== null}
    <div class="flex items-center justify-between gap-4 p-4 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700">
        <!-- Info Section -->
        <div class="text-sm text-slate-600 dark:text-slate-400">
            {#if state.totalItems !== null}
                Showing <span class="font-semibold text-slate-900 dark:text-slate-100">{state.totalItems}</span> items
            {/if}
            {#if state.hasMore}
