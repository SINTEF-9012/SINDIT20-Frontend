<script lang="ts">
	...
	const idSuffix =
		property.id ?? property.propertyName ?? Math.random().toString(36).slice(2, 10);
</script>
...
<div class="abstract-properties border variant-ghost-tertiary">
	<div class="prop">
		...
		<input
			...
			id={'name-' + idSuffix}
			name={'name-' + idSuffix}
			bind:value={inputForm.propertyName}
	...
	<div class="prop">
		...
		<input
			...
			name={'description-' + idSuffix}
			bind:value={inputForm.description}
			class="input text-white"
		/>
	</div>
	<div class="prop">
	...
	{#if property.propertyDataType}
		<div class="prop">
			...
			<input
				type="input"
				id={'dataType-' + idSuffix}
				name={'dataType-' + idSuffix}
				value={property.propertyDataType?.uri || property.propertyDataType}
				readonly
				class="input text-white"
			/>
		</div>
	...
	{#if property.propertyUnit}
		<div class="prop">
			...
			<input
				...
				id={'unit-' + idSuffix}
				name={'unit-' + idSuffix}
				value={property.propertyUnit?.uri || property.propertyUnit}
				readonly
				class="input text-white"
			/>
		</div>
	...
	{#if property.propertySemanticID}
		<div class="prop">
			...
			<input
				type="input"
				id={'semanticID-' + idSuffix}
				name={'semanticID-' + idSuffix}
				value={typeof property.propertySemanticID === 'object'
					? property.propertySemanticID.uri
					: property.propertySemanticID}
				readonly
				class="input text-white"
			/>
		</div>
	...
	{#if property.propertyConnection}
		<div class="prop">
			...
			<input
				type="input"
				id={'connection-' + idSuffix}
				name={'connection-' + idSuffix}
				value={property.propertyConnection?.uri || property.propertyConnection}
				readonly
				class="input text-white"
			/>
		</div>
